...
class PipelineConfig:
    def __init__(self, name, notebooks, source, storage, target):
...
def configure_pipeline(self, name, notebooks, source=False, storage=False, target=False):
    basepath = dbgems.get_notebook_dir()  # get filepath of current notebook        
    notebooks = [f"{basepath}/{n}" for n in notebooks]
    
    if not source: source = self.paths.stream_source
    if not storage: self.paths.storage_location
    if not target: self.schema_name
...
@DBAcademyHelper.monkey_patch    
def create_pipeline(self, config=False):
...
@DBAcademyHelper.monkey_patch
def validate_pipeline_config(self, display=True, config=False):
...
@DBAcademyHelper.monkey_patch
def start_pipeline(self, pipeline_id=False):
